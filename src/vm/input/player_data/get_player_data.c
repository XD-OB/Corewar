...
/*   Updated: 2020/01/24 01:41:57 by ishaimou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				c_size(int size, int valid_size)
{
	if (size != valid_size)
	{
		ft_dprintf(2, "error : missing informations in file\n");
		exit(EXIT_FAILURE);
	}
}
...
void				get_player_data(int fd, char *file)
{
	...
	c_size(read(fd, player->header.prog_name, PROG_NAME_LENGTH),
			PROG_NAME_LENGTH);
	c_size(read(fd, &delimiter, sizeof(delimiter)), sizeof(delimiter));
	if (delimiter != 0)
	...
	c_size(read(fd, player->header.comment, COMMENT_LENGTH), COMMENT_LENGTH);
	c_size(read(fd, &delimiter, sizeof(delimiter)), sizeof(delimiter));
	if (delimiter != 0)
